...
Client.prototype.createRelationship = function (fromID, toID, relationship, data, callback) {
    ...
    this.request('/node/' + fromID + '/relationships', options,
        responseHandler(201, errMsgs, getHeaderParser('location'), callback) );
};
...
Client.prototype.getRelationshipTypes = function (callback) {
    ...
    this.request('/relationships/types', options,
        responseHandler(200, errMsgs, getJsonParser(this.autoMarshal), callback) );
};

Client.prototype.getRelationships = function (id, dir, types, callback) {
    var options = {
        method: 'GET'
    };
    ...
    for(var i = 0; len = types.length; i < len; ++i) {
        types[i] = encodeURIComponent(types[i]);
    }
    var typesStr = (types.length > 0 ? "/" + types.join('&') : ''); 
    this.request('/node/' + id + '/relationships/' + dir + typesStr, options,
        responseHandler(200, errMsgs, getJsonParser(this.autoMarshal), callback) );
};
...
Client.prototype.updateRelationshipProperties = function (id, data, callback) {
    ...
    this.request('/relationship/' + id + '/properties', options,
        responseHandler(204, errMsgs, noOpParser, callback) );
};
...
Client.prototype.updateRelationshipProperty = function (id, property, value, callback) {
    ...
    this.request('/relationship/' + id + '/properties/' + encodeURIComponent(property), options,
        responseHandler(204, errMsgs, noOpParser, callback) );
};
...
Client.prototype.deleteRelationship = function (id, callback) {
    ...
    this.request('/relationship/' + id, options,
        responseHandler(204, errMsgs, noOpParser, callback) );
};
////////////////////////////////////////////////////////////////////////////////
// Utility Methods
